          ...
          <div class="card shadow mb-3">
               <div class="card-body">
                    <div class="table-responsive">
                         <table id="table-pemasok" class="table table-striped table-bordered table-hover">
                              <thead>
                              ...
                              <tbody>
                                   ...
                                   foreach ($pemasok as $p) : ?>
                                        <tr>
                                             ...
                                             <td><?= $p->alamat; ?></td>
                                             <td class="text-center">
                                                  <button class="btn btn-sm btn-warning" type="button" onclick="ubah(<?= $p->id; ?>)"><i class="fa fa-edit"></i> Ubah</button>
                                                  <?php if ($p->on_off == 1) : ?>
                                                       <button class="btn btn-sm btn-danger" type="button" disabled><i class="fa fa-trash"></i> Hapus</button>
                                                  <?php else : ?>
                                                       <button class="btn btn-sm btn-danger" type="button" onclick="hapus(<?= $p->id; ?>)"><i class="fa fa-trash"></i> Hapus</button>
                                                  <?php endif; ?>
                                             </td>
                                        </tr>
                                   <?php endforeach; ?>
...
<script>
     function tambah() {
          ...
          if (username != '' && password != '' && nama != '' && nohp != '' && alamat != '') {
               $.ajax({
                    url: "<?php echo base_url(); ?>Pemasok/tambah",
                    type: "POST",
                         ...
                         Swal.fire({
                              ...
                              if (data.status == 1) {
                                   ...
                                   Swal.fire({
                                        ...
                                        location.href = "<?php echo base_url() ?>Pemasok";
                                   });
...
<script>
     function ubah(id) {
          ...
          $.ajax({
               url: "<?php echo base_url(); ?>Pemasok/data/?id=" + id,
               type: "GET",
               dataType: "JSON",
               success: function(data) {
     ...
     function simpan() {
          ...
          if (nama != '' && nohp != '' && alamat != '') {
               $.ajax({
                    url: "<?php echo base_url(); ?>Pemasok/ubah",
                    type: "POST",
                         ...
                         if (data.status == 1) {
                              ...
                              Swal.fire({
                                   ...
                                   if (result.isConfirmed) {
                                        $('#modal-ubah').modal('hide');
                                        $.ajax({
                                             url: "<?php echo base_url(); ?>Pemasok/ubah_proses",
                                             type: "POST",
                                             data: ($('#form-ubah').serialize()),
                                             dataType: "JSON",
                                             success: function(data) {
     ...
     function hapus(id) {
          $.ajax({
               ...
               success: function(data) {
                    ...
                    $.ajax({
                         url: "<?php echo base_url(); ?>Pemasok/hapus/" + id,
                         type: "POST",
                         data: ($('#form-ubah').serialize()),
                         dataType: "JSON",
                         success: function(data) {
                              if (data.status == 1) {
                                   Swal.fire({
                                        title: 'HAPUS DATA',
                                        html: "Anda yakin ingin menghapus <br>" + username.toUpperCase().bold() + " ?",
                                        icon: 'warning',
                                        showCancelButton: true,
                                        confirmButtonColor: '#3085d6',
                                        cancelButtonColor: '#d33',
                                        ...
                                        if (result.isConfirmed) {
                                             $.ajax({
                                                  url: "<?php echo base_url(); ?>Pemasok/hapus_proses/" + id,
                                                  type: "POST",
                                                  data: ($('#form-ubah').serialize()),
                                                  dataType: "JSON",
                                                  success: function(data) {
