                              ...
                              <tbody>
                                   ...
                                   foreach ($pembelian as $p) : ?>
                                        <tr>
                                             ...
                                             <td><?= $p->jenis_ikan; ?></td>
                                             <td>Rp.
                                                  <span class="float-right"><?= number_format($p->harga_ikan); ?></span>
                                             </td>
                                             <td>
                                             ...
                                             <td class="text-center">
                                                  ...
                                                  <?php } else if ($p->status == 1) { ?>
                                                       <button class="btn btn-primary btn-sm" type="button" disabled><i class="fas fa-check-circle"></i> Diterima</button>
                                                  <?php } else { ?>
                                                       <button class="btn btn-success btn-sm" type="button" disabled><i class="fas fa-dollar-sign"></i> Selesai</button>
                                                  <?php } ?>
                                             </td>
                                             <td class="text-center">
                                                  <?php if ($p->status == 0) { ?>
                                                       <button class="btn btn-danger btn-sm" type="button" onclick="batalkan(<?= $p->id; ?>)"><i class="fas fa-times-circle"></i> Batal</button>
                                                  <?php } else if ($p->status == 1) { ?>
                                                       <button class="btn btn-info btn-sm" type="button" onclick="terima(<?= $p->id; ?>)"><i class="fas fa-check-circle"></i> Terima</button>
                                                  <?php } else { ?>
...
<script>
     function batalkan(id) {
          Swal.fire({
               ...
               html: 'Yakin ingin membatalkan pesanan ini ?',
               icon: 'question',
               showCancelButton: true,
               confirmButtonColor: '#3085d6',
               cancelButtonColor: '#d33',
               confirmButtonText: '<i class="fa fa-check-circle"></i> Batalkan',
               cancelButtonText: '<i class="fa fa-times-circle"></i> Tidak',
          }).then((result) => {
               if (result.isConfirmed) {
                    $.ajax({
                         url: "<?= site_url('Order/batal/') ?>" + id,
                         type: "GET",
                         dataType: "JSON",
